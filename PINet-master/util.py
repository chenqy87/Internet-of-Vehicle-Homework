...
def draw_points(x, y, image):
    ...
    for i in range(len(x)-1):
        array_0_x = np.array(x[i])[:,np.newaxis]
        array_0_y = np.array(y[i])[:,np.newaxis]
        array_1_x = np.array(x[i+1][::-1])[:,np.newaxis]
        array_1_y = np.array(y[i+1][::-1])[:,np.newaxis]
        array_0 = np.concatenate((array_0_x,array_0_y),axis = 1)
        array_1 = np.concatenate((array_1_x,array_1_y),axis = 1)
        array = np.concatenate((array_0,array_1),axis = 0)[np.newaxis,:,:]
        cv2.fillPoly(color_warp,array,p.color[i+1])
    image = cv2.addWeighted(image, 1, color_warp, 0.3, 0)
