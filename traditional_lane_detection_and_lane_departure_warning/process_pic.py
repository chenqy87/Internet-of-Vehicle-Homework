...
def create_output_frame(offcenter, pts, undist_ori, fps, curvature, curve_direction, output_frame_scale, 
    ...
    if abs(offcenter) > threshold:  # car is offcenter more than 0.6 m
        # Draw Red lane
        cv2.fillPoly(color_warp, np.int_([pts]), (0, 255, 0)) # red
...
#     y = np.floor(np.array([int(w_1*0.95),int(w_1*0.7),int(w_1*0.7),int(w_1*0.95)])/input_scale)
    x = np.floor(np.array([int(h_1*0),int(h_1*0.2),int(0.8*h_1),int(0.98*h_1)])/input_scale)
    y = np.floor(np.array([int(w_1*1),int(w_1*0.6),int(w_1*0.6),int(w_1*1)])/input_scale)
    x = x[:,np.newaxis]
    y = y[:,np.newaxis]
    src_0 = np.concatenate((x,y),axis = 1)
    src_2 = np.array(src_0,dtype = "int32")
    src_2 = src_2[np.newaxis,:,:]
#     cv2.polylines(img,src_2,True,(0,255,255),1)
    cv2.imshow("img",img)
#     src = np.float32([[0, 231], [30, 140], [240, 140], [320, 231]]) # 与ROI 保持一致
#     dst = np.float32([[10, 331], [20, 0], [310, 0], [300, 331]])
#     M = cv2.getPerspectiveTransform(src, dst)
#     Minv = cv2.getPerspectiveTransform(dst, src)
     
    X = np.floor(np.array([int(h_1*0.2),int(h_1*0),int(1*h_1),int(0.85*h_1)])/input_scale)
    Y = np.floor(np.array([int(w_1*1),int(w_1*0),int(w_1*0),int(w_1*1)])/input_scale)
...
#     Y = np.floor(np.array([int(w_1*0.95),int(w_1*0.5),int(w_1*0.5),int(w_1*0.95)])/input_scale)
    ...
    if left_lane.detected:  # start tracker
        right_lane,left_lane = tracker(binary_sub, right_lane, left_lane, ploty, input_scale, True)
    else:  # start detector
        right_lane,left_lane = detector(binary_sub, right_lane, left_lane, ploty, input_scale, True)
